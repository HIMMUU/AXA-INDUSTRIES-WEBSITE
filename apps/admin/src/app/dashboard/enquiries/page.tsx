'use client';

import { useQuery } from '@tanstack/react-query';
import { useEnquiriesStore } from '@/store/use-enquiries-store';
import { apiClient } from '@/lib/api-client';
import { Enquiry } from '@axa/types';
import { EnquiryFilters } from './components/enquiry-filters';
import { EnquiryTable } from './components/enquiry-table';
import { Download, RefreshCw, Inbox } from 'lucide-react';

export default function EnquiriesPage() {
  const {
    page,
    searchQuery,
    selectedStatus,
    selectedSource
  } = useEnquiriesStore();

  const {
    data,
    isLoading,
    refetch
  } = useQuery<{ items: Enquiry[]; meta: any }>({
    queryKey: ['enquiries', page, searchQuery, selectedStatus, selectedSource],
    queryFn: async () => {
      const params = new URLSearchParams({
        page: page.toString(),
        limit: '10',
        ...(searchQuery && { q: searchQuery }),
        ...(selectedStatus && { status: selectedStatus }),
        ...(selectedSource && { source: selectedSource })
      });

      const res = await apiClient(`/v1/enquiries?${params.toString()}`);
      const payload = res.data?.data || res.data || res;
      const items: Enquiry[] = Array.isArray(payload)
        ? payload
        : Array.isArray(payload?.items)
          ? payload.items
          : Array.isArray(payload?.data)
            ? payload.data
            : [];

      const meta = res.data?.meta || res.meta || payload?.meta || { page: 1, limit: 10, total: items.length, totalPages: 1 };
      return { items, meta };
    }
  });

  const handleExportAllCsv = async () => {
    try {
      const res = await fetch('http://localhost:4000/api/v1/enquiries/export', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('axa_access_token')}`
        }
      });
      const blob = await res.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `axa-all-enquiries-${new Date().toISOString().slice(0, 10)}.csv`;
      a.click();
    } catch (err) {
      console.error(err);
    }
  };

  return (
    <div className="space-y-6">
      {/* Top Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-white">Lead & Enquiry CRM</h2>
          <p className="text-xs text-neutral-400">Manage incoming quote requests, customer enquiries & conversion pipeline</p>
        </div>

        <div className="flex items-center gap-2">
          <button
            onClick={() => refetch()}
            className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-neutral-400 hover:text-white transition"
            title="Refresh Leads"
          >
            <RefreshCw className="h-4 w-4" />
          </button>
          <button
            onClick={handleExportAllCsv}
            className="flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/5 px-3.5 py-2 text-xs font-semibold text-white hover:bg-white/10 transition"
          >
            <Download className="h-3.5 w-3.5" />
            <span>Export CSV</span>
          </button>
        </div>
      </div>

      {/* Enquiry Filters */}
      <EnquiryFilters />

      {/* Enquiry Data Table */}
      <EnquiryTable
        enquiries={data?.items || []}
        meta={data?.meta || { page: 1, limit: 10, total: 0, totalPages: 1 }}
        isLoading={isLoading}
        refetch={refetch}
      />
    </div>
  );
}
